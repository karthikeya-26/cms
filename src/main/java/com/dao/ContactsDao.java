...
public class ContactsDao {
	...
	public ContactsObj getContactWithId(Integer contactId) throws DaoException {
		try {
			...
			getContact.table(Table.Contacts).condition(Contacts.CONTACT_ID, Operators.Equals, contactId.toString());
	...
	public ContactsObj getContactWithRefId(String refId) throws DaoException {
		try {
			Select s = new Select();
			s.table(Table.Contacts).condition(Contacts.REF_ID, Operators.Equals, refId);
			List<ResultObject> contacts = s.executeQuery(ContactsObj.class);
			
			return contacts.isEmpty()?null:(ContactsObj) contacts.get(0);
		} catch (QueryException e) {
	...
	public List<ContactsObj> getContactsWithUserId(Integer userId) throws DaoException {
		try {
			...
			Select s = new Select();
			s.table(Table.Contacts).condition(Contacts.USER_ID, Operators.Equals, userId.toString());
	...
	public int addContact(String firstName, String lastName, Integer userId) throws DaoException {
		try {
			Insert i = new Insert();
			i.table(Table.Contacts).columns(Contacts.FIRST_NAME, Contacts.LAST_NAME, Contacts.USER_ID).values(firstName,
	...
	public boolean updateContact(Integer contactId, String firstName, String lastName) throws DaoException {
		try {
			...
			updateContact.table(Table.Contacts).columns(Contacts.FIRST_NAME, Contacts.LAST_NAME)
					.values(firstName, lastName).condition(Contacts.CONTACT_ID, Operators.Equals, contactId.toString());
	...
	public boolean checkIfContactIsUsers(Integer contactId, Integer userId) throws DaoException {
		try {
			Select s = new Select();
			s.table(Table.Contacts).condition(Contacts.CONTACT_ID, Operators.Equals, contactId.toString())
	...
	public int syncAddContact(String firstName, String lastName, Integer userId, String resourceName,String refreshToken,
			long modifiedAt) throws DaoException{
		try {
			Insert addContact = new Insert();
			addContact.table(Table.Contacts).columns(Contacts.FIRST_NAME, Contacts.LAST_NAME, Contacts.USER_ID, Contacts.REF_ID,Contacts.REFRESH_TOKEN,Contacts.MODIFIED_AT);
			addContact.values(firstName,lastName,String.valueOf(userId), resourceName,refreshToken,String.valueOf(modifiedAt));
			return addContact.executeUpdate(true);
		}catch(QueryException e) {
			throw new DaoException("Failed to create contact");
		}
