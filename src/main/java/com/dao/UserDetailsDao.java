...
public class UserDetailsDao {
	...
	public UserDetailsObj getUserWithId(Integer userId) throws DaoException {
		...
		try {
			Select s = new Select();
			s.table(Table.UserDetails).condition(UserDetails.USER_ID, Operators.Equals, userId.toString());
	...
	public UserDetailsObj getUserWithMail(String mail) throws DaoException {
		...
		try {
			Select s = new Select();
			s.table(Table.UserDetails).columns(UserDetails.ALL_COLS).join(Joins.InnerJoin, Table.UserMails,
	...
	public UserDetailsObj getUserWithAccountId(String accountId) {
		...
		try {
			...
			getUserWithAcId.table(Table.UserDetails).condition(UserDetails.PROVIDER_AC_ID, Operators.Equals, accountId);
	...
	public int createUser(String userName, String firstName, String lastName, String contactType) throws DaoException {
		try {
			Insert i = new Insert();
			i.table(Table.UserDetails).columns(UserDetails.USER_NAME, UserDetails.FIRST_NAME, UserDetails.LAST_NAME,
					UserDetails.CONTACT_TYPE).values(userName, firstName, lastName, contactType);
	...
	public int createUser(String userName, String firstName, String lastName, String contactType, String accountId)
			throws DaoException {
		try {
			Insert i = new Insert();
