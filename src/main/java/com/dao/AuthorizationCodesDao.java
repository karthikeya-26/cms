...
public class AuthorizationCodesDao {
	...
	public AuthorizationCodesObj getAuthorizationCodesObj(String authCode) throws QueryException {
		...
		getAuthCode.table(Table.AuthorizationCodes).condition(AuthorizationCodes.AUTHORIZATION_CODE, Operators.Equals,
				authCode);
	...
	// INSERT
	public boolean addAuthorizationCode(String authorizationCode, String clientId, Integer userId, String scopes,
			String accessType) throws QueryException {
		Insert addCode = new Insert();
		addCode.table(Table.AuthorizationCodes)
				.columns(AuthorizationCodes.AUTHORIZATION_CODE, AuthorizationCodes.CLIENT_ID,
						AuthorizationCodes.USER_ID, AuthorizationCodes.SCOPES, AuthorizationCodes.ACCESS_TYPE)
	...
	public boolean deleteAuthCode(String authorizationCode) throws QueryException {
		...
		deleteCode.table(Table.AuthorizationCodes).condition(AuthorizationCodes.AUTHORIZATION_CODE, Operators.Equals,
				authorizationCode);
	...
	public boolean deleteUserAuthCodes(Integer userId) throws QueryException {
		...
		deleteUserCodes.table(Table.AuthorizationCodes).condition(AuthorizationCodes.USER_ID, Operators.Equals,
				userId.toString());
	...
	public boolean deleteClientCodes(Integer userId) throws QueryException {
		...
		deleteClientCodes.table(Table.AuthorizationCodes).condition(AuthorizationCodes.USER_ID, Operators.Equals,
				userId.toString());
